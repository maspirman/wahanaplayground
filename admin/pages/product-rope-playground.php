...
if (!empty($_GET['delete'])) {
  ...
  if ($p > 0) {
    echo "<script>
            window.location.href='index.php?p=product-rope-playground';
          </script>";
  } else {
    echo "<script>
...
<div class="card m-3">
  ...
  <div class="card-header border-0">
    <div class="row">
      <div class="col-6">
        <h3 class="mb-0">Table Rope Playground</h3>
      </div>
      <!-- Modal Button -->
  ...
  <!-- Light Table -->
  <?php $clasic = query("SELECT tbl_media.pict_name, tbl_media.alt, tbl_product.time FROM tbl_media INNER JOIN tbl_product ON tbl_media.id_media=tbl_product.id_media WHERE tbl_product.target='rope_playground' ORDER BY time DESC")[0]; ?>
        ...
        <?php foreach ($clasic as $r) : ?>
          <tr>
            ...
            <td class="table-actions">
              <a href="index.php?p=product-rope-playground&delete=<?= $r['time'] ?>" class="table-action table-action-delete" data-toggle="tooltip" data-original-title="Delete client">
